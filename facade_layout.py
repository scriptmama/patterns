...
def build_layout(
    ...
    panels.append(Panel("T_TOP", -1, -1, "trim", Rect(0, 0, width, trim_height)))
    for row in range(3):
        y = trim_height + row * panel_height
    ...
    panels.append(Panel("T_BOTTOM", 3, -1, "trim", Rect(0, trim_height + 3 * panel_height, width, trim_height)))
    return panels


def canvas_size(width: int = 1200, panel_height: int = 220, trim_height: int = 34) -> tuple[int, int]:
    return width, trim_height * 2 + panel_height * 3
